  ...
  return new Promise((resolve) => {
    ...
    ws.on('message', (raw) => {
      ...
      switch (packet.type) {
        ...
        case 'READY_UPDATE':
          // ⚠ 除錯用：只從房主視角印出每一筆 READY_UPDATE 的數字，方便確認
          //   房主到底有沒有收到「全員已準備」的那一筆，卡在哪一步一看就懂。
          if (isHost) {
          ...
          if (isHost && packet.payload?.readyCount >= packet.payload?.totalCount) {
            log(`房主判定全員已準備，${300}ms 後送出 GAME_START`);
        ...
        case 'DEPLOY_UPDATE':
          // ⚠ 除錯用：只從房主視角印，觀察 deployedCount 有沒有卡在 20 以下不動。
          if (isHost) {
        ...
        case 'ERROR':
          // ⚠ 新增：把伺服器回的 ERROR 印出來，避免像這次一樣悶頭等到逾時才發現
          //   是某個封包被拒絕（例如非房主送 GAME_START、點數超標等）。
