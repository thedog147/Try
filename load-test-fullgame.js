...
// ⚠ 新增：原本整場測試跑完可能才 10 幾秒，比 Render 30 秒一次的 [Memory]
//   快照週期還短，根本來不及在過程中捕捉到數據，只看得到「測試結束、大家都
//   離線」之後的低點。這個參數讓每次結束回合後刻意多等一下，把整場測試拉長，
//   確保至少能跨過 1~2 次 Memory 快照，才看得出「真正開打」時記憶體曲線的變化。
const EXTRA_TURN_DELAY_MS = parseInt(process.env.EXTRA_TURN_DELAY_MS || '0', 10);
...
// ⚠ 修正：40 人幾乎同時搶連線時，有些人的 JOIN 還沒送達伺服器、
//    先到位的人卻已經全部準備完成——這時候 totalCount（當下已加入人數）
//    只是一個持續往上爬的移動標的，不是最終的房間人數。原本的判斷「readyCount
//    >= 當下的 totalCount」在人數還沒到齊時也可能提早成立，而且每次有新人
//    加入又剛好準備完，就會被誤判成「全員到齊」而重複觸發。用一個跨連線共用
//    的旗標，確保整個測試流程中 GAME_START 只會真的送出一次。
...
function makeClient(idx) {
  return new Promise((resolve) => {
    ...
    let deployed = false;      // ⚠ 防呆：避免 GAME_START 意外重複觸發時，同一人重複生成單位
    let finished = false;
    let received = 0;
    ...
    const doTurnActions = () => {
      ...
      setTimeout(() => {
        ...
        // 結束回合
        setTimeout(() => send('END_TURN', {}), randDelay(200) + EXTRA_TURN_DELAY_MS);
      }, randDelay(300));
    };
    ...
    function handlePacket(packet) {
      ...
      switch (packet.type) {
        ...
        case 'READY_UPDATE':
          ...
          if (isHost && !gameStartSent && packet.payload?.readyCount >= PLAYER_COUNT) {
            // ⚠ 改用固定的 PLAYER_COUNT（腳本設定的目標人數）而不是當下還在
            //   變動的 totalCount，並用 gameStartSent 旗標確保只送出一次。
            gameStartSent = true;
            log(`房主判定全員（${PLAYER_COUNT}人）已準備，300ms 後送出 GAME_START`);
            setTimeout(() => send('GAME_START', { seed: 12345, mapType: 'north' }), 300);
          }
          break;

        case 'GAME_START':
          if (deployed) break;  // ⚠ 防呆：就算意外收到第二次也不再重複部署
          deployed = true;
